...
const AnimatedCounter = ({ target, duration = 4000, suffix = '', label = '' }) => {
  ...
  return (
    <div style={{ textAlign: 'center' }} >
      <div style={{ fontSize , color:'#fff' , fontWeight:'700', fontFamily: 'Bebas Neue'}}>
        {count}
        <span style={{ color: '#fff', fontFamily: 'Bebas Neue' }}>{suffix}</span>
      </div>
      <div className='labelFranja' style={{ marginTop: '0.25rem' }}>{label}</div>
    </div>
  );
};
